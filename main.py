...
MSG_ID_FILE = os.path.join(
    os.path.realpath(os.path.dirname(__file__)), "last_msg_id.txt"
)
...
def extract_videos_id(subject):
    ...
    for match in re.findall(
        r"youtube\.com/watch\?v=([\w-]+)|https://youtu\.be/([\w-]+)", subject
    ):
        m = m + match
...
@backoff.on_exception(
    ...
    on_backoff=lambda details: print(
        "Backing off {wait:0.1f} seconds after {tries} tries calling get_videos_id_playlist()".format(
            **details
        )
    ),
)
...
@backoff.on_exception(
    ...
    Exception,
    on_backoff=lambda details: print(
        "Backing off {wait:0.1f} seconds after {tries} tries calling get_discord_vids".format(
            **details
        )
    ),
)
def get_discord_vids(channelid, last_msg_id):
    ...
    url= f"https://discord.com/api/v9/channels/{channelid}/messages?after={last_msg_id}&limit=50"
...
@backoff.on_exception(
    ...
    on_backoff=lambda details: print(
        "Backing off {wait:0.1f} seconds after {tries} tries add_vids()".format(
            **details
        )
    ),
    max_tries=5,
...
def main():
    ...
    new_vids = list()
    last_msg_id = None
    thread_id = os.getenv("DISCORD_THREAD_ID", None)
    ...
    while True:
        ...
        print("Opening Discord Thread:", thread_id)
        new_vids, last_msg_id = get_vids(thread_id)
